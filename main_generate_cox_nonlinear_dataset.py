...
def get_nonlinear_cox_data(coefs: np.ndarray, size: int, censored_part=0.2, l=1e-5, v=2):
    ...
    event_times_nl = np.array([func(x[:, i]) for i, func in enumerate(funcs)]).sum(axis=0)
    event_times = (-math.log(0.5) / (l * np.exp(event_times_nl))) ** (1 / v)
...
res_dir = Path(
    f'bnam/cox_nonlinear={len(additive_funcs_all)}_fnum={len(additive_funcs_all[0])}_cl_size={cl_size}')
res_dir.mkdir(exist_ok=True, parents=True)

cox_clusters = [get_nonlinear_cox_data(coefs=cox_coefs, size=cl_size) for cox_coefs in additive_funcs_all]
